...
class App:
    ...
    def update(self):
        ...
        def update_student():
            query = '''
            UPDATE student 
            SET Surname=?, Name=?, DOB=?, Department=?, Gender=?, Level=?, Grade=?, Status=?, Faculty=?, Email=?, Phone_No=?, Time=?, Date=? 
            WHERE Matric_no=?
            '''
    ...
    def search_student(self):
        ...
        def searchStd():
            ...
            query='select *from student where Matric_no=? or Surname=? or Name=? or DOB=? or Department=? or Gender=? or Level=? or Grade=? or Status=? or Faculty=? or Email=? or Phone_No=?'
    ...
    def topsearch(self):
        ...
        query = 'SELECT * FROM Student WHERE Matric_no = ?'

        # Wrap `self.labeltopframeENTRY.get()` in a tuple to provide it as a parameter
        self.mycursor.execute(query, (self.labeltopframeENTRY.get().upper(),))
        fetched_data = self.mycursor.fetchall()
    ...
    def delete_student(self):
        ...
        query = 'DELETE FROM Student WHERE Matric_no = ?'

        # Pass data_matric as a single-element tuple
